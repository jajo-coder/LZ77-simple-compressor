...
int main(int argc, char* argv[]) {
	...
	if(!isFilePathWellFormatted(parameter, ".lz77")) return 1;
	...
	std::string decompressed = decompress(getCompressedObjectFromString(content));

	// writing this compressed content inside a file.lz77
	if(!createFileAndSetContent(parameter, decompressed)) return 1;
